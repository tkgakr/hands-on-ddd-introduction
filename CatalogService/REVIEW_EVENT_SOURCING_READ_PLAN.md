...
- [x] 書籍登録後、レビュー投稿し、推薦取得で投稿内容由来の推薦が返る。
- [x] レビュー編集後、推薦取得が編集後コメントを反映する。
- [x] レビュー削除後、推薦取得から該当レビューの推薦が消える。
- [x] SQL 実装でレビュー投稿後に推薦取得へ反映される。
- [x] SQL 実装でレビュー編集後に推薦取得へ反映される。
- [x] SQL 実装でレビュー削除後に推薦取得から除外される。
